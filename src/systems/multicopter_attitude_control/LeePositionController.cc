...
{
//////////////////////////////////////////////////
std::unique_ptr<LeePositionController> LeePositionController::MakeController(
    const LeePositionControllerParameters &_controllerParams,
    const VehicleParameters &_vehicleParams)
{
  // auto controller = std::make_unique<LeePositionController>();
  // Can't use make_unique here because the constructor is private
  std::unique_ptr<LeePositionController> controller(
      new LeePositionController());
...
//////////////////////////////////////////////////
bool LeePositionController::InitializeParameters()
{
    ...
    Eigen::VectorXd &_rotorVelocities) const
{
  Eigen::Vector3d acceleration =
      this->ComputeDesiredAcceleration(_frameData, _posYawDes);

  Eigen::Vector3d angularAcceleration =
      this->ComputeDesiredAngularAcc(_frameData, _posYawDes, acceleration);
...
//////////////////////////////////////////////////
Eigen::Vector3d LeePositionController::ComputeDesiredAcceleration(
                 const FrameData &_frameData, const EigenPositionYaw &_posYawDes) const
{
  Eigen::Vector3d positionError = _frameData.pose.translation() -
                                  _posYawDes.position;

  // The desired velocity for the position controller is 0
//   Eigen::Vector3d velocityError = _frameData.linearVelocityWorld -
//                                   _frameData.pose.linear() * _posYawDes.linear;
  Eigen::Vector3d velocityError = _frameData.linearVelocityWorld;
    ...
    const Eigen::Vector3d &_acceleration) const
{
  ...
  const double currentYaw = rot.eulerAngles(2, 1, 0)[2];
  ...
  // print accelCommand
  gzwarn << "angularRateDes: " << angularRateDes[2] << std::endl 
         << "currentYaw: " << currentYaw << std::endl
         << "desiredYaw: " << _posYawDes.yaw << std::endl;
