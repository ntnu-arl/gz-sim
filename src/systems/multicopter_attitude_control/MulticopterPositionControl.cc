...
#include "MulticopterPositionControl.hh"
...
//////////////////////////////////////////////////
void MulticopterPositionControl::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "MulticopterPositionControl plugin should be attached to a model "
  ...
  LeePositionControllerParameters controllerParameters;
  ...
  {
    gzerr << "Please specify positionGain for MulticopterPositionControl.\n";
    return;
  }
  ...
  {
    gzerr << "Please specify velocityGain for MulticopterPositionControl.\n";
    return;
  }
  ...
  {
    gzerr << "Please specify attitudeGain for MulticopterPositionControl.\n";
    return;
  }
  ...
  {
    gzerr << "Please specify angularRateGain MulticopterPositionControl.\n";
    return;
  }
  ...
  this->positionController = LeePositionController::MakeController(
      controllerParameters, vehicleParams);

  if (nullptr == this->positionController)
  {
    gzerr << "Error while creating the LeePositionController\n";
    return;
  }

  math::Vector3d positionMean{0, 0, 0};
  sdfClone->Get<math::Vector3d>("positionNoiseMean", positionMean,
      positionMean);
  ...
  this->noiseParameters.positionMean = math::eigen3::convert(positionMean);
  ...
  this->node.Subscribe(topic, &MulticopterPositionControl::OnPose, this);
  gzmsg << "MulticopterPositionControl subscribing to Pose messages on ["
         << topic << "]" << std::endl;

  std::string enableTopic{this->robotNamespace + "/" + this->enableSubTopic};
  this->node.Subscribe(enableTopic, &MulticopterPositionControl::OnEnable,
                       this);
  gzmsg << "MulticopterPositionControl subscribing to Boolean messages on ["
...
//////////////////////////////////////////////////
math::Inertiald MulticopterPositionControl::VehicleInertial(
...
//////////////////////////////////////////////////
void MulticopterPositionControl::PreUpdate(
    const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("MulticopterPositionControl::PreUpdate");
  ...
  EigenPositionYaw positionYaw;
  {
    ...
    Eigen::Quaternion quat = Eigen::Quaterniond(this->poseMsg->orientation().w(),
                                                this->poseMsg->orientation().x(),
                                                this->poseMsg->orientation().y(),
                                                this->poseMsg->orientation().z());
    positionYaw.yaw = quat.toRotationMatrix().eulerAngles(2, 1, 0)[2];

    gzwarn << "MulticopterPositionControl:\n position: " << positionYaw.position
           <<"\n"<< " yaw: " << positionYaw.yaw << std::endl;
  }
  ...
  this->positionController->CalculateRotorVelocities(*frameData, positionYaw,
...
//////////////////////////////////////////////////
void MulticopterPositionControl::OnPose(
...
//////////////////////////////////////////////////
void MulticopterPositionControl::OnEnable(
...
//////////////////////////////////////////////////
void MulticopterPositionControl::PublishRotorVelocities(
    EntityComponentManager &_ecm,
...
GZ_ADD_PLUGIN(MulticopterPositionControl,
                    System,
                    MulticopterPositionControl::ISystemConfigure,
                    MulticopterPositionControl::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(
    MulticopterPositionControl,
    "gz::sim::systems::MulticopterPositionControl")

// TODO(CH3): Deprecated, remove on version 8
GZ_ADD_PLUGIN_ALIAS(
    MulticopterPositionControl,
    "ignition::gazebo::systems::MulticopterPositionControl")
