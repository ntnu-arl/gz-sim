...
{
//////////////////////////////////////////////////
std::unique_ptr<LeeAccelerationController> LeeAccelerationController::MakeController(
    const LeeAccelerationControllerParameters &_controllerParams,
    const VehicleParameters &_vehicleParams)
{
  // auto controller = std::make_unique<LeeAccelerationController>();
  // Can't use make_unique here because the constructor is private
  std::unique_ptr<LeeAccelerationController> controller(
      new LeeAccelerationController());
...
//////////////////////////////////////////////////
bool LeeAccelerationController::InitializeParameters()
{
...
//////////////////////////////////////////////////
void LeeAccelerationController::CalculateRotorVelocities(
    const FrameData &_frameData, const EigenTwist &_cmdAcc,
    Eigen::VectorXd &_rotorVelocities) const
{
  Eigen::Vector3d acceleration =
      this->ComputeDesiredAcceleration(_frameData, _cmdAcc);

  Eigen::Vector3d angularAcceleration =
      this->ComputeDesiredAngularAcc(_frameData, _cmdAcc, acceleration);
...
//////////////////////////////////////////////////
Eigen::Vector3d LeeAccelerationController::ComputeDesiredAcceleration(
                 const FrameData &_frameData, const EigenTwist &_cmdAcc) const
{
  // the acceleration command is probably clipped to the maximum before getting here
  Eigen::Vector3d accelCommand = Eigen::Vector3d::Zero() - _frameData.pose.linear() * _cmdAcc.linear;
...
// SE(3)
Eigen::Vector3d LeeAccelerationController::ComputeDesiredAngularAcc(
    const FrameData &_frameData, const EigenTwist &_cmdAcc,
    const Eigen::Vector3d &_acceleration) const
{
  ...
  angularRateDes[2] = _cmdAcc.angular[2];
