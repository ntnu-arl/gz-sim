...
#include "MulticopterAccelerationControl.hh"
...
//////////////////////////////////////////////////
void MulticopterAccelerationControl::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "MulticopterAccelerationControl plugin should be attached to a model "
  ...
  LeeAccelerationControllerParameters controllerParameters;
  ...
  {
    gzerr << "Please specify attitudeGain for MulticopterAccelerationControl.\n";
    return;
  }
  ...
  {
    gzerr << "Please specify angularRateGain MulticopterAccelerationControl.\n";
    return;
  }

  if (sdfClone->HasElement("maximumLinearAcceleration"))
  {
    this->maximumLinearAcceleration =
        sdfClone->Get<math::Vector3d>("maximumLinearAcceleration").Abs();
  ...
  this->accelerationController = LeeAccelerationController::MakeController(
      controllerParameters, vehicleParams);

  if (nullptr == this->accelerationController)
  {
    gzerr << "Error while creating the LeeAccelerationController\n";
    return;
  }
  ...
  this->node.Subscribe(topic, &MulticopterAccelerationControl::OnTwist, this);
  gzmsg << "MulticopterAccelerationControl subscribing to Twist messages on ["
         << topic << "]" << std::endl;

  std::string enableTopic{this->robotNamespace + "/" + this->enableSubTopic};
  this->node.Subscribe(enableTopic, &MulticopterAccelerationControl::OnEnable,
                       this);
  gzmsg << "MulticopterAccelerationControl subscribing to Boolean messages on ["
...
//////////////////////////////////////////////////
math::Inertiald MulticopterAccelerationControl::VehicleInertial(
...
//////////////////////////////////////////////////
void MulticopterAccelerationControl::PreUpdate(
    const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("MulticopterAccelerationControl::PreUpdate");
  ...
  if (!this->controllerActive)
  {
    ...
    if (this->rotorVelocities.squaredNorm() > 0)
    {
      ...
      // Clear the cmdAccMsg so that the system waits for a new command after
      // being renabled.
      std::lock_guard<std::mutex> lock(this->cmdAccMsgMutex);
      this->cmdAccMsg.reset();
    }
    return;
  }
  ...
  {
    std::lock_guard<std::mutex> lock(this->cmdAccMsgMutex);
    if (!this->cmdAccMsg.has_value())
    {
      return;
    }

    // Clip with max linear acceleration
    math::Vector3d linear = msgs::Convert(this->cmdAccMsg->linear());
    linear.Min(this->maximumLinearAcceleration);
    linear.Max(-this->maximumLinearAcceleration);

    math::Vector3d angular = msgs::Convert(this->cmdAccMsg->angular());
    angular.Min(this->maximumAngularVelocity);
    angular.Max(-this->maximumAngularVelocity);

    cmdAcc.linear = math::eigen3::convert(linear);
    cmdAcc.angular = math::eigen3::convert(angular);
  }
  ...
  this->accelerationController->CalculateRotorVelocities(*frameData, cmdAcc,
...
//////////////////////////////////////////////////
void MulticopterAccelerationControl::OnTwist(
...
//////////////////////////////////////////////////
void MulticopterAccelerationControl::OnEnable(
...
//////////////////////////////////////////////////
void MulticopterAccelerationControl::PublishRotorVelocities(
    EntityComponentManager &_ecm,
...
GZ_ADD_PLUGIN(MulticopterAccelerationControl,
                    System,
                    MulticopterAccelerationControl::ISystemConfigure,
                    MulticopterAccelerationControl::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(
    MulticopterAccelerationControl,
    "gz::sim::systems::MulticopterAccelerationControl")

// TODO(CH3): Deprecated, remove on version 8
GZ_ADD_PLUGIN_ALIAS(
    MulticopterAccelerationControl,
    "ignition::gazebo::systems::MulticopterAccelerationControl")
